...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_task_stage.name_plural'), docType:'ctlg_task_stage'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_task_stage.name_plural')" :listDeletedTitle="$t('ctlg_task_stage.name_plural_deleted')" pg-method="ctlg_task_stage_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_task_stage_update"
