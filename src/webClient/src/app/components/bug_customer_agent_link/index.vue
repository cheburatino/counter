...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('bug_customer_agent_link.name_plural'), docType:'bug_customer_agent_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('bug_customer_agent_link.name_plural')" :listDeletedTitle="$t('bug_customer_agent_link.name_plural_deleted')" pg-method="bug_customer_agent_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="bug_customer_agent_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/bug_customer_agent_link/',
    },
    data() {
      return {
        listSortData: [
