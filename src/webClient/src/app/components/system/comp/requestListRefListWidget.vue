<template>
    <div>
        <q-bar class="bg-secondary text-white shadow-2">
            <div>запросы <span v-if="deleted">удаленные</span></div>
            <q-space />
            <q-btn icon="add" v-if="!readonly" round flat @click="add"><q-tooltip>Добавить</q-tooltip></q-btn>
            <q-btn v-if="deleted && !readonly" icon="delete" round flat @click="reload(false)"><q-tooltip>активные запросы</q-tooltip></q-btn>
            <q-btn v-if="!deleted && !readonly" icon="delete_outline" round flat @click="reload(true)"><q-tooltip>удаленные запросы</q-tooltip></q-btn>
        </q-bar>

        <q-list bordered separator>
            <q-item v-for="v in list" :key="v.id">
                <router-link :to="'/request/' + v.id" style="cursor: pointer">
    ...
    export default {
        ...
        methods: {
            ...
            reload(isDeleted) {
                ...
                this.$utils.callPgMethod('request_list', {'system_id': this.id, deleted: this.deleted, 'order_by': 'created_at desc', }, (result) => this.list = result)
            },
            saveNew() {
                ...
                // если IsStateMachine то request_create, в остальных случаях request_update
                this.$utils.callPgMethod('request_update', params, () => {
            ...
            removeRecover({id}) {
                this.$utils.callPgMethod('request_update', {id, deleted: !this.deleted}, () => this.reload(this.deleted))
            }
        },
        mounted() {
            this.reload()
