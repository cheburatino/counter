...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('task_specialist_link.name_plural'), docType:'task_specialist_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('task_specialist_link.name_plural')" :listDeletedTitle="$t('task_specialist_link.name_plural_deleted')" pg-method="task_specialist_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="task_specialist_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/task_specialist_link/',
    },
    data() {
      return {
        listSortData: [
