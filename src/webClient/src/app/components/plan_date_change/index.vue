...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('plan_date_change.name_plural'), docType:'plan_date_change'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('plan_date_change.name_plural')" :listDeletedTitle="$t('plan_date_change.name_plural_deleted')" pg-method="plan_date_change_list"
      ...
      <template #listItem="{item}">
		...
		<router-link :to="currentUrl + item.id" style="cursor: pointer">
			<q-item-section avatar>
			  <q-avatar rounded>
				<img src="image/plan_date_change.png" alt="">
			  </q-avatar>
			</q-item-section>
		</router-link>
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="plan_date_change_update"
