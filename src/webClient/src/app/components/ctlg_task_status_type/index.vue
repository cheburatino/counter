...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_task_status_type.name_plural'), docType:'ctlg_task_status_type'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_task_status_type.name_plural')" :listDeletedTitle="$t('ctlg_task_status_type.name_plural_deleted')" pg-method="ctlg_task_status_type_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_task_status_type_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/ctlg_task_status_type/',
    },
    data() {
      return {
        listSortData: [
