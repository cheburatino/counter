<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb v-if="!isOpenInDialog" :list="[{label:'Цели клиентов', docType:'client_purpose'}]"/>

    <comp-doc-list ref="docList" listTitle='Цели клиентов' listDeletedTitle='Удаленные цели клиентов' pg-method="client_purpose_list"
      ...
      <template #listItem="{item}">
		...
		<router-link :to="currentUrl + item.id" style="cursor: pointer">
			<q-item-section avatar>
			  <q-avatar rounded>
				<img src="image/client_purpose.svg" alt="">
			  </q-avatar>
			</q-item-section>
		</router-link>
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="client_purpose_update"
