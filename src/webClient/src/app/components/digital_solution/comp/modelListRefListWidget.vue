<template>
    <div>
        <q-bar class="bg-secondary text-white shadow-2">
            <div>модели <span v-if="deleted">удаленные</span></div>
            <q-space />
            <q-btn icon="add" v-if="!readonly" round flat @click="add"><q-tooltip>Добавить</q-tooltip></q-btn>
            <q-btn v-if="deleted && !readonly" icon="delete" round flat @click="reload(false)"><q-tooltip>активные модели</q-tooltip></q-btn>
            <q-btn v-if="!deleted && !readonly" icon="delete_outline" round flat @click="reload(true)"><q-tooltip>удаленные модели</q-tooltip></q-btn>
        </q-bar>

        <q-list bordered separator>
            <q-item v-for="v in list" :key="v.id">
                <router-link :to="'/model/' + v.id" style="cursor: pointer">
                    <q-item-section avatar>
                        <q-avatar rounded>
                            <img src="image/model.svg" alt="">
                        </q-avatar>
                    </q-item-section>
                </router-link>
                <q-item-section>
    ...
    export default {
        ...
        methods: {
            ...
            reload(isDeleted) {
                ...
                this.$utils.callPgMethod('model_list', {'digital_solution_id': this.id, deleted: this.deleted, 'order_by': 'created_at desc', }, (result) => this.list = result)
            },
            saveNew() {
                ...
                // если IsStateMachine то model_create, в остальных случаях model_update
                this.$utils.callPgMethod('model_update', params, () => {
            ...
            removeRecover({id}) {
                this.$utils.callPgMethod('model_update', {id, deleted: !this.deleted}, () => this.reload(this.deleted))
            }
        },
        mounted() {
            this.reload()
