...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('program_client.name_plural'), docType:'program_client'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('program_client.name_plural')" :listDeletedTitle="$t('program_client.name_plural_deleted')" pg-method="program_client_list"
      ...
      <template #listItem="{item}">
		...
		<router-link :to="currentUrl + item.id" style="cursor: pointer">
			<q-item-section avatar>
			  <q-avatar rounded>
				<img src="image/program_client.png" alt="">
			  </q-avatar>
			</q-item-section>
		</router-link>
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="program_client_update"
