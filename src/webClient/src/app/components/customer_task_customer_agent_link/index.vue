...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('customer_task_customer_agent_link.name_plural'), docType:'customer_task_customer_agent_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('customer_task_customer_agent_link.name_plural')" :listDeletedTitle="$t('customer_task_customer_agent_link.name_plural_deleted')" pg-method="customer_task_customer_agent_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="customer_task_customer_agent_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/customer_task_customer_agent_link/',
    },
    data() {
      return {
        listSortData: [
