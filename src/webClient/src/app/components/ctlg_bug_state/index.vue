...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_bug_state.name_plural'), docType:'ctlg_bug_state'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_bug_state.name_plural')" :listDeletedTitle="$t('ctlg_bug_state.name_plural_deleted')" pg-method="ctlg_bug_state_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_bug_state_update"
