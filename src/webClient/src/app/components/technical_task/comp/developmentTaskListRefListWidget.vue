    ...
    <div>
        ...
        <q-list bordered separator>
            <q-item v-for="v in list" :key="v.id">
                <router-link :to="'/development_task/' + v.id" style="cursor: pointer">
    ...
    export default {
        ...
        methods: {
            ...
            reload(isDeleted) {
                ...
                this.$utils.callPgMethod('development_task_list', {'technical_task_id': this.id, deleted: this.deleted, 'order_by': 'created_at desc', }, (result) => this.list = result)
            },
            saveNew() {
                ...
                // если IsStateMachine то development_task_create, в остальных случаях development_task_update
                this.$utils.callPgMethod('development_task_update', params, () => {
            ...
            removeRecover({id}) {
                this.$utils.callPgMethod('development_task_update', {id, deleted: !this.deleted}, () => this.reload(this.deleted))
            }
        },
        mounted() {
            this.reload()
