...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('digital_solution_specialist_link.name_plural'), docType:'digital_solution_specialist_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('digital_solution_specialist_link.name_plural')" :listDeletedTitle="$t('digital_solution_specialist_link.name_plural_deleted')" pg-method="digital_solution_specialist_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="digital_solution_specialist_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/digital_solution_specialist_link/',
    },
    data() {
      return {
        listSortData: [
