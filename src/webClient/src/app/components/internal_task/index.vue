<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb v-if="!isOpenInDialog" :list="[{label:'Внутренние задачи', docType:'internal_task'}]"/>

    <comp-doc-list ref="docList" listTitle='Внутренние задачи' listDeletedTitle='Удаленные внутренние задачи' pg-method="internal_task_list"
      ...
      <template #listItem="{item}">
		...
		<router-link :to="currentUrl + item.id" style="cursor: pointer">
			<q-item-section avatar>
			  <q-avatar rounded>
				<img src="statics/image/internal_task.svg" alt="">
			  </q-avatar>
			</q-item-section>
		</router-link>
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="internal_task_update"
