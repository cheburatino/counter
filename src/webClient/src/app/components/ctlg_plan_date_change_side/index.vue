...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_plan_date_change_side.name_plural'), docType:'ctlg_plan_date_change_side'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_plan_date_change_side.name_plural')" :listDeletedTitle="$t('ctlg_plan_date_change_side.name_plural_deleted')" pg-method="ctlg_plan_date_change_side_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_plan_date_change_side_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/ctlg_plan_date_change_side/',
    },
    data() {
      return {
        listSortData: [
