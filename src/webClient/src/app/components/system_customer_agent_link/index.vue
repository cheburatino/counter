...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('system_customer_agent_link.name_plural'), docType:'system_customer_agent_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('system_customer_agent_link.name_plural')" :listDeletedTitle="$t('system_customer_agent_link.name_plural_deleted')" pg-method="system_customer_agent_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="system_customer_agent_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/system_customer_agent_link/',
    },
    data() {
      return {
        listSortData: [
