...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_request_priority.name_plural'), docType:'ctlg_request_priority'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_request_priority.name_plural')" :listDeletedTitle="$t('ctlg_request_priority.name_plural_deleted')" pg-method="ctlg_request_priority_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_request_priority_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/ctlg_request_priority/',
    },
    data() {
      return {
        listSortData: [
