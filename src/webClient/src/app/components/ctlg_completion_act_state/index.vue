...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_completion_act_state.name_plural'), docType:'ctlg_completion_act_state'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_completion_act_state.name_plural')" :listDeletedTitle="$t('ctlg_completion_act_state.name_plural_deleted')" pg-method="ctlg_completion_act_state_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_completion_act_state_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/ctlg_completion_act_state/',
    },
    data() {
      return {
        listSortData: [
