...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('ctlg_functional_requirement_state.name_plural'), docType:'ctlg_functional_requirement_state'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('ctlg_functional_requirement_state.name_plural')" :listDeletedTitle="$t('ctlg_functional_requirement_state.name_plural_deleted')" pg-method="ctlg_functional_requirement_state_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="ctlg_functional_requirement_state_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/ctlg_functional_requirement_state/',
    },
    data() {
      return {
        listSortData: [
