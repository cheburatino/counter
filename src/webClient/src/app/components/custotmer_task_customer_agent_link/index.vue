...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('custotmer_task_customer_agent_link.name_plural'), docType:'custotmer_task_customer_agent_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('custotmer_task_customer_agent_link.name_plural')" :listDeletedTitle="$t('custotmer_task_customer_agent_link.name_plural_deleted')" pg-method="custotmer_task_customer_agent_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="custotmer_task_customer_agent_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/custotmer_task_customer_agent_link/',
    },
    data() {
      return {
        listSortData: [
