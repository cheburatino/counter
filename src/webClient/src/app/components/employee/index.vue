...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb v-if="!isOpenInDialog" :list="[{label:'Сотрудники', docType:'employee'}]"/>

    <comp-doc-list ref="docList" listTitle='Сотрудники' listDeletedTitle='Удаленные сотрудники' pg-method="employee_list"
      ...
      <template #listItem="{item}">
		...
		<router-link :to="currentUrl + item.id" style="cursor: pointer">
			<q-item-section avatar>
			  <q-avatar rounded>
				<img src="image/employee.svg" alt="">
			  </q-avatar>
			</q-item-section>
		</router-link>
