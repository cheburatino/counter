...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('work_specialist_link.name_plural'), docType:'work_specialist_link'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('work_specialist_link.name_plural')" :listDeletedTitle="$t('work_specialist_link.name_plural_deleted')" pg-method="work_specialist_link_list"
      ...
      <template #listItem="{item}">
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="work_specialist_link_update"
...
<script>
  ...
  export default {
    ...
    computed: {
      currentUrl: () => '/work_specialist_link/',
    },
    data() {
      return {
        listSortData: [
