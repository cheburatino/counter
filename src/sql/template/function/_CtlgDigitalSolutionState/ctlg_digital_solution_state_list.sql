-- получение списка статус цифрового решения
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_digital_solution_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_digital_solution_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_digital_solution_state as doc ' || condQueryStr || ')
