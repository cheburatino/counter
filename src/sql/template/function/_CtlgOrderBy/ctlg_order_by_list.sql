...
DROP FUNCTION IF EXISTS ctlg_order_by_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_order_by_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'user_table_id', 'doc.user_table_id'],
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_order_by as doc ' || condQueryStr || ')
