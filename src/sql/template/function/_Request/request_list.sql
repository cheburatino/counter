...
DROP FUNCTION IF EXISTS request_list(params JSONB);
CREATE OR REPLACE FUNCTION request_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'customer_agent_id', 'doc.customer_agent_id'],
    ...
    if is_user_role((params->>'user_id')::int, '{admin}') is not true then
        ...
        whereStr = whereStr || format(' AND id = ANY(%s) ', quote_literal((select coalesce(array_agg(request_id), '{}') from request_customer_agent_link where customer_agent_id = (select id from man where user_table_id = (params->>'user_id')::int) and deleted=false)));
    end if;

    -- показываем только активные
    whereStr = whereStr || ' and state_id != 5';
    ...
    EXECUTE ('
	with t1 as (select * from request as doc ' || condQueryStr || ')
