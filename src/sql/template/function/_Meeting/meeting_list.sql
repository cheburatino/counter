...
DROP FUNCTION IF EXISTS meeting_list(params JSONB);
CREATE OR REPLACE FUNCTION meeting_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from meeting as doc ' || condQueryStr || ')
