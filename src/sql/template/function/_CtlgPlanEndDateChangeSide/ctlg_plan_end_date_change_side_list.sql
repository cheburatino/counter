-- получение списка из-за кого изменилась плановая дата завершения
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_plan_end_date_change_side_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_plan_end_date_change_side_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_plan_end_date_change_side as doc ' || condQueryStr || ')
