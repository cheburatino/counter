-- функция триггер
DROP FUNCTION IF EXISTS technical_task_trigger_after() CASCADE;
CREATE OR REPLACE FUNCTION technical_task_trigger_after() RETURNS trigger AS
$$
DECLARE
        r record;
        jsonbEl      jsonb;
BEGIN
        
IF (TG_OP = 'UPDATE') THEN
-- при смене названия обновляем все ссылающиеся записи, чтобы там переписалось новое название
if new.title != old.title then
 for r in select * from development_task where technical_task_id = new.id loop
 update development_task set updated_at=now() where id = r.id;
 end loop;
 for r in select * from task where technical_task_id = new.id loop
 update task set updated_at=now() where id = r.id;
 end loop;
 for r in select * from invoice where technical_task_id = new.id loop
 update invoice set updated_at=now() where id = r.id;
 end loop;
 for r in select * from completion_act where technical_task_id = new.id loop
 update completion_act set updated_at=now() where id = r.id;
 end loop;

 end if;
 end if;

        

        

    RETURN NEW;
END;

$$ LANGUAGE plpgsql;

