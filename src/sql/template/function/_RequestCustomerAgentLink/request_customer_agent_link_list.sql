-- получение списка связь запроса и представителя заказчика
-- параметры:
...
DROP FUNCTION IF EXISTS request_customer_agent_link_list(params JSONB);
CREATE OR REPLACE FUNCTION request_customer_agent_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from request_customer_agent_link as doc ' || condQueryStr || ')
