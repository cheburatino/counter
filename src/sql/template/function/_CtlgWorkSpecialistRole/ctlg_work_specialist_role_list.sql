-- получение списка роль специалиста в деле
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_work_specialist_role_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_work_specialist_role_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_work_specialist_role as doc ' || condQueryStr || ')
