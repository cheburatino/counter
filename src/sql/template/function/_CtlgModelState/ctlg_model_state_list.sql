-- получение списка статус модели
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_model_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_model_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_model_state as doc ' || condQueryStr || ')
