-- получение списка участник задачи
-- параметры:
...
DROP FUNCTION IF EXISTS task_specialist_link_list(params JSONB);
CREATE OR REPLACE FUNCTION task_specialist_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from task_specialist_link as doc ' || condQueryStr || ')
