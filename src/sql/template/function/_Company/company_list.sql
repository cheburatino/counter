...
DROP FUNCTION IF EXISTS company_list(params JSONB);
CREATE OR REPLACE FUNCTION company_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from company as doc ' || condQueryStr || ')
