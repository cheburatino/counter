-- получение списка связь компании и человека
-- параметры:
...
DROP FUNCTION IF EXISTS company_man_link_list(params JSONB);
CREATE OR REPLACE FUNCTION company_man_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from company_man_link as doc ' || condQueryStr || ')
