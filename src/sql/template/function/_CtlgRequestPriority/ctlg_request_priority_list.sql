-- получение списка приоритет запроса
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_request_priority_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_request_priority_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_request_priority as doc ' || condQueryStr || ')
