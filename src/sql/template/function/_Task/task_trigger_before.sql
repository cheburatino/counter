...
BEGIN
        ...
        NEW.options = coalesce(OLD.options, '{}'::jsonb) || NEW.options || jsonb_build_object('title', jsonb_build_object('title', new.title, 'digital_solution_title', digitalSolutionTitle, 'type_title', typeTitle, 'author_title', authorTitle, 'director_title', directorTitle, 'executor_title', executorTitle, 'acceptor_title', acceptorTitle));
        -- заполняем search_text
        
        NEW.search_text = concat(new.title, ' ', digitalSolutionTitle, ' ', typeTitle, ' ', authorTitle, ' ', directorTitle, ' ', executorTitle, ' ', acceptorTitle, ' ', searchTxtVar);
