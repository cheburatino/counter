...
BEGIN
    ...
    with t1 as (select * from task where id = (params ->> 'id')::int),
		...
		t5 as (select t4.*, c.title as type_title from t4 left join ctlg_task_type c on c.id = t4.type_id),
		...
		t9 as (select t8.*, c.title as executor_title from t8 left join man c on c.id = t8.executor_id)
