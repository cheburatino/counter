-- получение списка связь функционального требования и специалиста
-- параметры:
...
DROP FUNCTION IF EXISTS functional_requirement_specialist_link_list(params JSONB);
CREATE OR REPLACE FUNCTION functional_requirement_specialist_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from functional_requirement_specialist_link as doc ' || condQueryStr || ')
