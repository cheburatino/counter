-- получение списка участник дела
-- параметры:
...
DROP FUNCTION IF EXISTS work_specialist_link_list(params JSONB);
CREATE OR REPLACE FUNCTION work_specialist_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from work_specialist_link as doc ' || condQueryStr || ')
