-- получение списка программный клиент
-- параметры:
...
DROP FUNCTION IF EXISTS program_client_list(params JSONB);
CREATE OR REPLACE FUNCTION program_client_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from program_client as doc ' || condQueryStr || ')
