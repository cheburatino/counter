...
DROP FUNCTION IF EXISTS ctlg_electron_skill_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_electron_skill_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_electron_skill as doc ' || condQueryStr || ')
