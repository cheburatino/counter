-- получение списка новость от Димы
-- параметры:
...
DROP FUNCTION IF EXISTS news_from_dima_list(params JSONB);
CREATE OR REPLACE FUNCTION news_from_dima_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from news_from_dima as doc ' || condQueryStr || ')
