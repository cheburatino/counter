-- получение списка задача по разработке
-- параметры:
...
DROP FUNCTION IF EXISTS development_task_list(params JSONB);
CREATE OR REPLACE FUNCTION development_task_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from development_task as doc ' || condQueryStr || ')
