...
BEGIN
    ...
    with t1 as (select * from development_task where id = (params ->> 'id')::int),
		...
		t5 as (select t4.*, c.title as technical_task_title from t4 left join technical_task c on c.id = t4.technical_task_id),
		t6 as (select t5.*, c.title as digital_solution_title from t5 left join digital_solution c on c.id = t5.digital_solution_id),
		t7 as (select t6.*, c.title as responsible_title from t6 left join man c on c.id = t6.responsible_id),
		t8 as (select t7.*, c.title as sprint_title from t7 left join sprint c on c.id = t7.sprint_id)
