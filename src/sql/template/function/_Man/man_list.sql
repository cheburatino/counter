...
DROP FUNCTION IF EXISTS man_list(params JSONB);
CREATE OR REPLACE FUNCTION man_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from man as doc ' || condQueryStr || ')
