...
DROP FUNCTION IF EXISTS ctlg_filter_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_filter_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_filter as doc ' || condQueryStr || ')
