...
DROP FUNCTION IF EXISTS contact_list(params JSONB);
CREATE OR REPLACE FUNCTION contact_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from contact as doc ' || condQueryStr || ')
