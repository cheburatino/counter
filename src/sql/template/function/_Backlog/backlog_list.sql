...
DROP FUNCTION IF EXISTS backlog_list(params JSONB);
CREATE OR REPLACE FUNCTION backlog_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from backlog as doc ' || condQueryStr || ')
