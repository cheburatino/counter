-- получение списка роль специалиста в функциональном требовании
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_functional_requirement_specialist_role_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_functional_requirement_specialist_role_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_functional_requirement_specialist_role as doc ' || condQueryStr || ')
