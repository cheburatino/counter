-- получение списка статус задачи на разработку
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_development_task_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_development_task_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_development_task_state as doc ' || condQueryStr || ')
