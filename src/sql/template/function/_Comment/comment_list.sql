...
DROP FUNCTION IF EXISTS comment_list(params JSONB);
CREATE OR REPLACE FUNCTION comment_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from comment as doc ' || condQueryStr || ')
