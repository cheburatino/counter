-- получение списка изменение плановой даты
-- параметры:
...
DROP FUNCTION IF EXISTS plan_date_change_list(params JSONB);
CREATE OR REPLACE FUNCTION plan_date_change_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from plan_date_change as doc ' || condQueryStr || ')
