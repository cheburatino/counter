-- функция триггер
DROP FUNCTION IF EXISTS plan_date_change_trigger_before() CASCADE;
CREATE OR REPLACE FUNCTION plan_date_change_trigger_before() RETURNS trigger AS
$$
DECLARE
        r record;
	taskTitle TEXT;
...
BEGIN
		...
		select title into sideTitle from ctlg_plan_date_change_side where id = new.side_id;
