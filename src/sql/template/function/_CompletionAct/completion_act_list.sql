-- получение списка акт выполненных работ
-- параметры:
...
DROP FUNCTION IF EXISTS completion_act_list(params JSONB);
CREATE OR REPLACE FUNCTION completion_act_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from completion_act as doc ' || condQueryStr || ')
