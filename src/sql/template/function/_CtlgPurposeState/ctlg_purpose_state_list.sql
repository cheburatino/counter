...
DROP FUNCTION IF EXISTS ctlg_purpose_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_purpose_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_purpose_state as doc ' || condQueryStr || ')
