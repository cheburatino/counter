-- получение списка связь цифрового решения и заказчика
-- параметры:
...
DROP FUNCTION IF EXISTS digital_solution_customer_agent_link_list(params JSONB);
CREATE OR REPLACE FUNCTION digital_solution_customer_agent_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from digital_solution_customer_agent_link as doc ' || condQueryStr || ')
