-- получение списка статус задачи разработки
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_dev_task_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_dev_task_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_dev_task_state as doc ' || condQueryStr || ')
