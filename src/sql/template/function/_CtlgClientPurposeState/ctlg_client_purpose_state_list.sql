-- получение списка статус цели клиента
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_client_purpose_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_client_purpose_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_client_purpose_state as doc ' || condQueryStr || ')
