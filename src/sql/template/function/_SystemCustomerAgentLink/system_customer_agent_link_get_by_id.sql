-- поиск связь системы и представителя заказчика по id
-- параметры:
-- id       type: int

DROP FUNCTION IF EXISTS system_customer_agent_link_get_by_id(params JSONB);
CREATE OR REPLACE FUNCTION system_customer_agent_link_get_by_id(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    checkMsg = check_required_params_with_func_name('system_customer_agent_link_get_by_id', params, ARRAY ['id']);
    IF checkMsg IS NOT NULL
    THEN
        RETURN checkMsg;
    END IF;

    with t1 as (select * from system_customer_agent_link where id = (params ->> 'id')::int),
