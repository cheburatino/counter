-- получение списка связь системы и представителя заказчика
-- параметры:
...
DROP FUNCTION IF EXISTS system_customer_agent_link_list(params JSONB);
CREATE OR REPLACE FUNCTION system_customer_agent_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from system_customer_agent_link as doc ' || condQueryStr || ')
