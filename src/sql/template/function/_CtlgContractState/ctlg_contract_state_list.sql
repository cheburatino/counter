-- получение списка статус договора
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_contract_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_contract_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_contract_state as doc ' || condQueryStr || ')
