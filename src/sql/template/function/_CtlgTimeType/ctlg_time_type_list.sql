...
DROP FUNCTION IF EXISTS ctlg_time_type_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_time_type_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_time_type as doc ' || condQueryStr || ')
