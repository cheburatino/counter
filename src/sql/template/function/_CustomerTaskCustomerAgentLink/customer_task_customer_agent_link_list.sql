...
DROP FUNCTION IF EXISTS customer_task_customer_agent_link_list(params JSONB);
CREATE OR REPLACE FUNCTION customer_task_customer_agent_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'customer_task_id', 'doc.customer_task_id'],
    ...
    EXECUTE ('
	with t1 as (select * from customer_task_customer_agent_link as doc ' || condQueryStr || ')
