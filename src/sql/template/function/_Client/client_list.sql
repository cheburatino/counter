...
DROP FUNCTION IF EXISTS client_list(params JSONB);
CREATE OR REPLACE FUNCTION client_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from client as doc ' || condQueryStr || ')
