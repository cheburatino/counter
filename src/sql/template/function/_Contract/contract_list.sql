...
DROP FUNCTION IF EXISTS contract_list(params JSONB);
CREATE OR REPLACE FUNCTION contract_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'counterparty_id', 'doc.counterparty_id']
    ]);
    ...
    EXECUTE ('
	with t1 as (select * from contract as doc ' || condQueryStr || ')
