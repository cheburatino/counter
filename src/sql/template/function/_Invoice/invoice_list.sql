...
DROP FUNCTION IF EXISTS invoice_list(params JSONB);
CREATE OR REPLACE FUNCTION invoice_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from invoice as doc ' || condQueryStr || ')
