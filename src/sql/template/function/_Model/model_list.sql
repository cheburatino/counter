...
DROP FUNCTION IF EXISTS model_list(params JSONB);
CREATE OR REPLACE FUNCTION model_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from model as doc ' || condQueryStr || ')
