-- получение списка статус системы
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_system_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_system_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_system_state as doc ' || condQueryStr || ')
