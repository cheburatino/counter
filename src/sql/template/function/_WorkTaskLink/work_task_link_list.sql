-- получение списка связь дела и задачи
-- параметры:
...
DROP FUNCTION IF EXISTS work_task_link_list(params JSONB);
CREATE OR REPLACE FUNCTION work_task_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from work_task_link as doc ' || condQueryStr || ')
