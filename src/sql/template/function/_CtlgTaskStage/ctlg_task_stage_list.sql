...
DROP FUNCTION IF EXISTS ctlg_task_stage_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_task_stage_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_task_stage as doc ' || condQueryStr || ')
