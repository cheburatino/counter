-- получение списка роль специалиста в цифровом решении
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_digital_solution_specialist_role_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_digital_solution_specialist_role_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_digital_solution_specialist_role as doc ' || condQueryStr || ')
