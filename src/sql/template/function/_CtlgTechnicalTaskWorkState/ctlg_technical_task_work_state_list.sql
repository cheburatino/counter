-- получение списка статус работ по ТЗ
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_technical_task_work_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_technical_task_work_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_technical_task_work_state as doc ' || condQueryStr || ')
