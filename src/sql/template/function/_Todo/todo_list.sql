...
DROP FUNCTION IF EXISTS todo_list(params JSONB);
CREATE OR REPLACE FUNCTION todo_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'performer_id', 'doc.performer_id'],
    ...
    EXECUTE ('
	with t1 as (select * from todo as doc ' || condQueryStr || ')
