...
DROP FUNCTION IF EXISTS counterparty_list(params JSONB);
CREATE OR REPLACE FUNCTION counterparty_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from counterparty as doc ' || condQueryStr || ')
