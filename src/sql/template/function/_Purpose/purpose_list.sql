...
DROP FUNCTION IF EXISTS purpose_list(params JSONB);
CREATE OR REPLACE FUNCTION purpose_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from purpose as doc ' || condQueryStr || ')
