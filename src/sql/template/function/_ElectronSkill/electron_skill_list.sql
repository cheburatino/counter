...
DROP FUNCTION IF EXISTS electron_skill_list(params JSONB);
CREATE OR REPLACE FUNCTION electron_skill_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from electron_skill as doc ' || condQueryStr || ')
