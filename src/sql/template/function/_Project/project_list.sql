...
DROP FUNCTION IF EXISTS project_list(params JSONB);
CREATE OR REPLACE FUNCTION project_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from project as doc ' || condQueryStr || ')
