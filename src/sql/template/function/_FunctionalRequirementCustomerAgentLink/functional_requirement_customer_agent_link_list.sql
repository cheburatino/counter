-- получение списка связь функционального требования и представителя заказчика
-- параметры:
...
DROP FUNCTION IF EXISTS functional_requirement_customer_agent_link_list(params JSONB);
CREATE OR REPLACE FUNCTION functional_requirement_customer_agent_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from functional_requirement_customer_agent_link as doc ' || condQueryStr || ')
