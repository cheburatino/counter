-- получение списка статус запроса
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_request_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_request_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_request_state as doc ' || condQueryStr || ')
