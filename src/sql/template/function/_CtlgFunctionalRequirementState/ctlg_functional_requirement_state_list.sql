-- получение списка статус функционального требования
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_functional_requirement_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_functional_requirement_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_functional_requirement_state as doc ' || condQueryStr || ')
