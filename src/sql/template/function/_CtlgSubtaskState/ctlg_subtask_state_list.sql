-- получение списка статус подзадачи
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_subtask_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_subtask_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_subtask_state as doc ' || condQueryStr || ')
