-- получение списка связь задачи заказчика и представителя заказчика
-- параметры:
...
DROP FUNCTION IF EXISTS custotmer_task_customer_agent_link_list(params JSONB);
CREATE OR REPLACE FUNCTION custotmer_task_customer_agent_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'custotmer_task_id', 'doc.custotmer_task_id'],
    ...
    EXECUTE ('
	with t1 as (select * from custotmer_task_customer_agent_link as doc ' || condQueryStr || ')
