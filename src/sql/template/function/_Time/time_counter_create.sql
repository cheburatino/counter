...
drop function if exists time_counter_create(params jsonb);
create or replace function time_counter_create(params jsonb)
    returns jsonb
    language plpgsql
as
$function$
...
begin
    ...
    if timeRow notnull then raise exception 'уже существует активное время по работе. ID Времени: %', timeRow.id; end if;
    ...
    if not (updateRes->>'ok')::bool then raise exception 'Ошибка создания счётчика в time_counter_create: %', updateRes->>'message'; end if;
