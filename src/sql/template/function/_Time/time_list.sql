...
DROP FUNCTION IF EXISTS time_list(params JSONB);
CREATE OR REPLACE FUNCTION time_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from time as doc ' || condQueryStr || ')
