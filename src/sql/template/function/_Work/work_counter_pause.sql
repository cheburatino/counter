...
drop function if exists work_counter_pause(params jsonb);
create or replace function work_counter_pause(params jsonb)
    returns jsonb
    language plpgsql
as
$function$
...
begin
    ...
    if timeRow.counter_start_time isnull then
        updateRes = time_update(jsonb_build_object(
                ...
                'effort', coalesce((EXTRACT(EPOCH FROM now()) - EXTRACT(EPOCH FROM timeRow.start_time)) / 60, 0) + timeRow.effort,
                'state_id', 2
            ));
    ...
    if timeRow.counter_start_time notnull then
        updateRes = time_update(jsonb_build_object(
                'id', timeRow.id,
                'work_id', timeRow.work_id,
                'effort', ((EXTRACT(EPOCH FROM now()) - EXTRACT(EPOCH FROM timeRow.counter_start_time)) / 60) + timeRow.effort,
                'state_id', 2
            ));
        if not (updateRes->>'ok')::bool then raise exception 'Ошибка остановки счётчика в work_counter_continue: %', updateRes->>'message'; end if;
    end if;
