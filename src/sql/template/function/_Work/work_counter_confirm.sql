...
drop function if exists work_counter_confirm(params jsonb);
create or replace function work_counter_confirm(params jsonb)
    returns jsonb
    language plpgsql
as
$function$
...
begin
    ...
    if not (updateRes->>'ok')::bool then raise exception 'Ошибка завершения счётчика в work_counter_continue: %', updateRes->>'message'; end if;
