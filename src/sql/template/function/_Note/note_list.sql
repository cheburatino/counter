...
DROP FUNCTION IF EXISTS note_list(params JSONB);
CREATE OR REPLACE FUNCTION note_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from note as doc ' || condQueryStr || ')
