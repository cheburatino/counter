...
DROP FUNCTION IF EXISTS work_time_sheet_list(params JSONB);
CREATE OR REPLACE FUNCTION work_time_sheet_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from work_time_sheet as doc ' || condQueryStr || ')
