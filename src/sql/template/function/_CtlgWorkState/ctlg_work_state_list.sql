-- получение списка статус работы
-- параметры:
...
DROP FUNCTION IF EXISTS ctlg_work_state_list(params JSONB);
CREATE OR REPLACE FUNCTION ctlg_work_state_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from ctlg_work_state as doc ' || condQueryStr || ')
