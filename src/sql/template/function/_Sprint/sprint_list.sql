...
DROP FUNCTION IF EXISTS sprint_list(params JSONB);
CREATE OR REPLACE FUNCTION sprint_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from sprint as doc ' || condQueryStr || ')
