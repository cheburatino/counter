...
DROP FUNCTION IF EXISTS employee_list(params JSONB);
CREATE OR REPLACE FUNCTION employee_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from employee as doc ' || condQueryStr || ')
