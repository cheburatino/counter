-- получение списка связь цифрового решения и специалиста
-- параметры:
...
DROP FUNCTION IF EXISTS digital_solution_specialist_link_list(params JSONB);
CREATE OR REPLACE FUNCTION digital_solution_specialist_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from digital_solution_specialist_link as doc ' || condQueryStr || ')
