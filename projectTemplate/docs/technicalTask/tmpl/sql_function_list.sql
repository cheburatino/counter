-- получение списка техническое задание
-- параметры:
...
DROP FUNCTION IF EXISTS technical_task_list(params JSONB);
CREATE OR REPLACE FUNCTION technical_task_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'work_state_id', 'doc.work_state_id'],
		['notQuoted', 'contract_id', 'doc.contract_id']
    ]);
    ...
    EXECUTE ('
	with t1 as (select * from technical_task as doc ' || condQueryStr || ')
