...
func GetDoc(project *t.ProjectType) t.DocType {
	doc := t.DocType{
		...
		Flds: []t.FldType{
			t.GetFldTitle(),
			t.GetFldRef("system_id", "система", "system", [][]int{{1, 2}},  "col-2", "isShowLink", "isClearable").SetReadonly("currentUser.role?.includes(`customer`)"),
			t.GetFldRef("state_id", "статус", "ctlg_bug_state", [][]int{{1, 3}}, "col-2").SetReadonly("currentUser.role?.includes(`customer`)"),
			t.GetFldRef("sprint_id", "спринт", "sprint", [][]int{{2, 1}}, "col-2", "isShowLink", "isClearable"),
			t.GetFldInt("estimate", "оценка", [][]int{{2, 2}}, "col-1"),
			t.GetFldInt("worked_time", "затрачено", [][]int{{2, 3}}, "col-1"),
			t.GetFldDate("plan_end_date", "плановая дата завершения", [][]int{{2, 4}}, "col-2"),
			t.GetFldDate("fact_end_date", "фактическая дата завершения", [][]int{{2, 5}}, "col-2"),
