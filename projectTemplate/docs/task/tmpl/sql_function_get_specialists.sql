...
DROP FUNCTION IF EXISTS task_get_specialists(params JSONB);
CREATE OR REPLACE FUNCTION task_get_specialists(params JSONB)
    RETURNS JSONB
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    with t1 as (select * from man where company_id = 1)
