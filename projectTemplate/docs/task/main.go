...
func GetDoc(project *t.ProjectType) t.DocType {
	doc := t.DocType{
		...
		Flds: []t.FldType{
			t.GetFldTitle(),
			t.GetFldRef("type_id", "тип задачи", "ctlg_task_type", [][]int{{1, 2}}, "col-2").SetIsRequired(),
			...
			t.GetFldRef("customer_id", "заказчик", "company", [][]int{{6, 1}}),
			t.GetFldRef("system_id", "система", "system", [][]int{{6, 2}}),
			t.GetFldRef("digital_solution_id", "цифровое решение", "digital_solution", [][]int{{7, 1}}, "col-4", "isShowLink"),
			t.GetFldRef("functional_requirement_id", "функциональное требование", "functional_requirement", [][]int{{7, 2}}),
			t.GetFldRef("bug_id", "баг", "bug", [][]int{{8, 1}}),
