...
<!--      <q-icon name="fas fa-atom" />-->
    ...
    <q-dialog v-model="isShowDialog" persistent>
      <q-card style="min-width: 250px">
        <q-card-section>
          <div style="text-align: center" class="text-h6">Данные по специалисту</div>
        </q-card-section>
        ...
        <q-btn-dropdown color="primary" label="Роль" style="margin-left: 40px; margin-bottom: 10px; margin-top: 20px;">
          <q-list>
            <q-item clickable v-close-popup @click="onRoleClick(item)" v-for="item in roleList" :key="item.title">
              <q-item-section>
                <q-item-label>{{ item.title }}</q-item-label>
              </q-item-section>
            </q-item>
        ...
        <div v-if="role" style="margin-left: 40px; border-bottom: 1px solid #4f6ccd; margin-right: 40px;">
          {{role}}
        </div>
...
export default {
  ...
  methods: {
    ...
    editPosition(id) {
      ...
      this.roleId = activetaskSpec.role_id
      this.spec = activetaskSpec.options.title.specialist_title
      this.role = activetaskSpec.options.title.role_title
    ...
    reloadList () {
      this.$utils.postCallPgMethod({
        method: 'work_specialist_link_list',
        params: {
          work_id: this.item.id
        }
      }).subscribe(v => {
  ...
  mounted() {
    ...
    this.$utils.postCallPgMethod({
      method: 'ctlg_work_specialist_role_list',
      params: {}
    }).subscribe(v => {
      if (v.ok) {
