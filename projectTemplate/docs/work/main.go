...
func GetDoc(project *t.ProjectType) t.DocType {
	doc := t.DocType{
		...
		Flds: []t.FldType{
			...
			t.GetFldInt("worked_time", "затрачено минут", [][]int{{1, 2}}, "col-2"),
			t.GetFldRef("state_id", "статус", "ctlg_work_state", [][]int{{1, 3}}, "col-2").SetDefault("1"),
			...
			t.GetFldRef("meeting_id", "встреча", "meeting", [][]int{{2, 3}}, "col-2", "isShowLink", "isClearable"),
			t.GetFldCheckbox("is_paid", "оплачиваемая", [][]int{{2, 4}}, "col-2").SetDefault("true"),
			...
			t.GetFldString("process", "процесс", 0, [][]int{{5, 1}}, "col-8"),
			t.GetFldFiles("process_files", "файлы результата", [][]int{{6, 1}}, t.FldVueFilesParams{}),
			t.GetFldImgList("process_images", "изображения результата", [][]int{{6, 2}}, t.FldVueImgParams{}),
